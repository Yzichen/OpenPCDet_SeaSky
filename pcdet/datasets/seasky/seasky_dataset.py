...
class SeaSkyDataset(DatasetTemplate):
    ...
    def include_data(self, mode):
        self.logger.info('Loading SeaSky dataset.')
        seasky_infos = []
        ...
        for info_path in self.dataset_cfg.INFO_PATH[mode]:
            ...
            with open(info_path, 'rb') as f:
                infos = pickle.load(f)
                seasky_infos.extend(infos)

        self.seasky_infos.extend(seasky_infos)
        self.logger.info('Total samples for SeaSky dataset: %d' % (len(seasky_infos)))
    ...
    def get_label(self, idx):
        ...
        assert label_file.exists(), f"{label_file}不存在"
        with open(label_file, 'r') as f:
            lines = f.readlines()

        if len(lines) == 0:
            return np.zeros((0, 7), dtype=np.float32), np.zeros((0, ), dtype=np.str_)
    ...
    def __len__(self):
        ...
        return len(self.seasky_infos)

    def __getitem__(self, index):
        if self._merge_all_iters_to_one_epoch:
            index = index % len(self.seasky_infos)

        info = copy.deepcopy(self.seasky_infos[index])
    ...
    def evaluation(self, det_annos, class_names, **kwargs):
        if 'annos' not in self.seasky_infos[0].keys():
        ...
        eval_gt_annos = [copy.deepcopy(info['annos']) for info in self.seasky_infos]
    ...
    def create_groundtruth_database(self, info_path=None, used_classes=None, split='train'):
        ...
        db_info_save_path = Path(self.root_path) / ('seasky_dbinfos_%s.pkl' % split)
...
def create_seasky_infos(dataset_cfg, class_names, data_path, save_path, workers=4):
    ...
    train_filename = save_path / ('seasky_infos_%s.pkl' % train_split)
    val_filename = save_path / ('seasky_infos_%s.pkl' % test_split)

    print('------------------------Start to generate data infos------------------------')

    dataset.set_split(train_split)
    print(dataset.root_split_path)
    seasky_infos_train = dataset.get_infos(
        class_names, num_workers=workers, has_label=True, num_features=num_features
    )
    with open(train_filename, 'wb') as f:
        pickle.dump(seasky_infos_train, f)
    print('SeaSky info train file is save to %s' % train_filename)

    dataset.set_split(test_split)
    print(dataset.root_split_path)
    seasky_infos_val = dataset.get_infos(
        class_names, num_workers=workers, has_label=True, num_features=num_features
    )
    with open(val_filename, 'wb') as f:
        pickle.dump(seasky_infos_val, f)
    print('SeaSky info train file is save to %s' % val_filename)
...
if __name__ == '__main__':
    ...
    if sys.argv.__len__() > 1 and sys.argv[1] == 'create_seasky_infos':
        import yaml
        from pathlib import Path
        from easydict import EasyDict

        dataset_cfg = EasyDict(yaml.safe_load(open(sys.argv[2])))
        ROOT_DIR = Path("/home/zichen/Documents/202DATA/LidarSet")
        create_seasky_infos(
            dataset_cfg=dataset_cfg,
            class_names=['bus', 'train', 'car', 'pedestrian', 'truck', 'motor', 'bicycle'],
            data_path=ROOT_DIR,
            save_path=ROOT_DIR,
        )
